...
from source.util import create_data_url
...
def restyle_with_responses_api(client, from_image_path, to_image_path, 
    output_path, size, style_prompt=None):
    """Restyles a photo using the style of another image 
    via the Responses API and the gpt-5.4-mini model."""
    ...
    response = client.responses.create(
        ...
        input=[{
            ...
            'content': [
                ...
                {'type': 'input_image',
                 'image_url': create_data_url(to_image_path)},
                {'type': 'input_image',
                 'image_url': create_data_url(from_image_path)},
            ],
        }],
    )
